...
public class CompteEstalvi {
    ...
    public int removeUser(String dni) throws BankAccountException {
        ...
        if(comprobarLast()) throw new BankAccountException(BankAccountException.ACCOUNT_LAST);
        else{
    ...
    public void comprobarCompte(Double salado) throws BankAccountException {

        if(salado < 0) throw new BankAccountException(BankAccountException.ACCOUNT_OVERDRAFT);


    }

    public boolean comprobarLast(){

        if(llista_usuaris.size() == 1) return true;
        else return false;
    }
}
